...
interface CreativesTableActionBarProps {
  table: Table<Creative>
}

export function CreativesTableActionBar({
  table,
}: CreativesTableActionBarProps) {
  ...
  const onDelete = React.useCallback(async () => {
    try {
      await Promise.all(
        rows.map((row) =>
          deleteCreative.mutateAsync({ id: row.original.id })
        )
      )
      table.toggleAllRowsSelected(false)
