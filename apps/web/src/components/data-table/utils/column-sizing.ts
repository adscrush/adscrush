 ...
 */
function hasIdAndSize<TData>(column: ColumnDef<TData, unknown>): column is ColumnDef<TData, unknown> & { id: string; size: number } {
  return "id" in column && typeof column.id === "string" && "size" in column && typeof column.size === "number";
}

/**
 * Type guard to check if column has accessorKey and size
 */
function hasAccessorKeyAndSize<TData>(column: ColumnDef<TData, unknown>): column is ColumnDef<TData, unknown> & { accessorKey: string; size: number } {
  return "accessorKey" in column && typeof column.accessorKey === "string" && "size" in column && typeof column.size === "number";
}

/**
 * Extract default column sizes from column definitions
 */
export function extractDefaultColumnSizes<TData>(columns: ColumnDef<TData, unknown>[]): Record<string, number> {
...
function isValidColumnSizing(value: unknown): value is Record<string, number> {
  ...
  return Object.values(value as Record<string, unknown>).every((size) => typeof size === "number" && !Number.isNaN(size) && size > 0);
}

/**
 * Initialize column sizes from localStorage or defaults
 */
export function initializeColumnSizes<TData>(columns: ColumnDef<TData, unknown>[], tableId: string, setColumnSizing: (sizes: Record<string, number>) => void): void {
