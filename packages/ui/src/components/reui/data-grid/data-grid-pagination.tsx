    ...
    >
      <div className="order-2 flex flex-wrap items-center space-x-2.5 pb-2.5 sm:order-1 sm:pb-0">
        {isLoading ? (
          ...
          <>
            <div className="text-xs/relaxed text-muted-foreground">{mergedProps.rowsPerPageLabel}</div>
            <Select
              value={`${pageSize}`}
              onValueChange={(value) => {
      ...
      <div className="order-1 flex flex-col items-center justify-center gap-2.5 pt-2.5 sm:order-2 sm:flex-row sm:justify-end sm:pt-0">
        {isLoading ? (
          ...
          <>
            <div className="order-2 text-xs/relaxed text-nowrap text-muted-foreground sm:order-1">{paginationInfo}</div>
            {pageCount > 1 && (
