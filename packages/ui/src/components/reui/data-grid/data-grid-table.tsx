...
import { useDataGrid } from "@adscrush/ui/components/reui/data-grid/data-grid"
...
function getPinningStyles<TData>(column: Column<TData>, resizable?: boolean): CSSProperties {
  const isPinned = column.getIsPinned()

  return {
    left:
      isPinned === "left"
        ? `calc(var(--col-${column.id}-left, 0) * 1px)`
        : undefined,
    right:
      isPinned === "right"
        ? `calc(var(--col-${column.id}-right, 0) * 1px)`
        : undefined,
    position: isPinned ? "sticky" : "relative",
    width: resizable ? `calc(var(--col-${column.id}-size) * 1px)` : column.getSize(),
...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  // Use visual pinned order (left → center → right) so <col> elements map
  // positionally to the same <th>/<td> order rendered by getHeaderGroups().
  const visibleColumns = useMemo(
    () => [
      ...table.getLeftLeafColumns().filter((c) => c.getIsVisible()),
      ...table.getCenterLeafColumns().filter((c) => c.getIsVisible()),
      ...table.getRightLeafColumns().filter((c) => c.getIsVisible()),
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [table.getState().columnPinning, table.getState().columnVisibility, table.getState().columnOrder]
  )
  ...
  const columnSizeVars = useMemo(() => {
    ...
    // 1. Widths for all visible leaf columns
    const visibleLeafColumns = table.getVisibleLeafColumns()
    visibleLeafColumns.forEach((column) => {
      colSizes[`--col-${column.id}-size`] = column.getSize()
    })

    // 2. Offsets for left pinned columns (visual order)
    const leftPinned = table.getLeftLeafColumns()
    let leftOffset = 0
    leftPinned.forEach((column) => {
      colSizes[`--col-${column.id}-left`] = leftOffset
      leftOffset += column.getSize()
    })

    // 3. Offsets for right pinned columns (visual order)
    const rightPinned = [...table.getRightLeafColumns()].reverse()
    let rightOffset = 0
    rightPinned.forEach((column) => {
      colSizes[`--col-${column.id}-right`] = rightOffset
  ...
  return (
    <table
      ...
      className={cn(
        "caption-bottom text-left align-middle text-xs/relaxed font-normal text-foreground rtl:text-right",
...
function DataGridTableHeadRowCell<TData>({
  ...
  // Visual index within the header group (left-pinned first, then center, then right-pinned).
  const headerVisualIndex = header.headerGroup.headers.indexOf(header)
    ...
    <th
      ...
      style={{
        ...
        ...(props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          getPinningStyles(column, props.tableLayout?.columnsResizable)),
        ...(props.tableLayout?.columnsResizable && {
          width: `calc(var(--col-${column.id}-size) * 1px)`,
        }),
      }}
      ...
      className={cn(
        "relative h-8 text-left align-middle font-normal text-secondary-foreground/80 rtl:text-right [&:has([role=checkbox])]:pe-0",
        headerCellSpacing,
        props.tableLayout?.cellBorder && "border-e",
        props.tableLayout?.columnsResizable && column.getCanResize() && "overflow-visible",
        props.tableLayout?.columnsResizable && column.getCanResize() && headerVisualIndex === header.headerGroup.headers.length - 1 && "pe-8",
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
        ...
        // Use visual position in the header group (not original column index) for edge detection.
        headerVisualIndex === 0 || headerVisualIndex === header.headerGroup.headers.length - 1
...
function DataGridTableHeadRowCellResize<TData>({ header }: { header: Header<TData, unknown> }) {
  const { props, table } = useDataGrid()
  const { column } = header
  // Use the header's position within its header group (visual order) instead of
  // column.getIndex() which returns the original pre-pinning index.
  const headerVisualIndex = header.headerGroup.headers.indexOf(header)
  const isLastVisibleColumn = headerVisualIndex === header.headerGroup.headers.length - 1
...
function DataGridTableResizeIndicator({ viewportElement }: { viewportElement: HTMLDivElement | null }) {
  ...
  // Calculate visual start position by summing widths of preceding headers in the same row
  const headerGroup = resizingHeader.headerGroup
  const headerIndex = headerGroup.headers.indexOf(resizingHeader)
  const visualStart = headerGroup.headers.slice(0, headerIndex).reduce((acc, h) => acc + h.getSize(), 0)
  ...
  const indicatorLeft =
    typeof columnSizingInfo.startOffset === "number" && viewportElement
      ...
      : visualStart + resizingHeader.getSize()
  ...
  return (
    <tbody
      className={cn(
        ...
        props.tableLayout?.rowRounded && "[&_td:first-child]:rounded-l-4xl",
        props.tableLayout?.rowRounded && "[&_td:last-child]:rounded-r-4xl",
...
function DataGridTableFootRow({ children }: { children: ReactNode }) {
  const { props } = useDataGrid()
  return (
    <tr className={cn("bg-muted/40 [&>td:first-child]:bg-muted/30", props.tableLayout?.cellBorder && "*:last:border-e-0")}>
      {children}
  ...
  return (
    <td
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          column?.getCanPin() &&
          "data-pinned:bg-muted/90 data-pinned:backdrop-blur-xs [&[data-pinned=left][data-last-col=left]]:border-e! [&[data-pinned=right][data-last-col=right]]:border-s! [&[data-pinned][data-last-col]]:border-border",
        className
      )}
    >
      {children}
    ...
    <td
      ...
      className={cn(
        ...
        props.tableLayout?.columnsPinnable &&
          column.getCanPin() &&
          "data-pinned:backdrop-blur-xs data-pinned:bg-background/90 [&[data-pinned=left][data-last-col=left]]:border-e! [&[data-pinned=right][data-last-col=right]]:border-s! [&[data-pinned][data-last-col]]:border-border",
        // Use cell's visual index in getVisibleCells() for edge detection.
        (() => { const cells = row.getVisibleCells(); const idx = cells.findIndex(c => c.id === cell.id); return idx === 0 || idx === cells.length - 1 ? props.tableClassNames?.edgeCell : "" })()
      )}
    >
      {children}
    </td>
...
function DataGridTableEmpty() {
  ...
  return (
    <tr>
      <td colSpan={Math.max(visibleColumnCount, 1)} className="py-6 text-center text-xs/relaxed text-muted-foreground">
...
function DataGridTableLoader() {
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      <div className="flex items-center gap-2 rounded-md border bg-card px-4 py-2 text-xs/relaxed leading-none font-medium text-muted-foreground">
...
function DataGridTableRowSelect<TData>({ row }: { row: Row<TData> }) {
  return (
    <>
      <div
        className={cn("inset-s-0 absolute top-0 bottom-0 hidden w-[2px] bg-primary", row.getIsSelected() && "block")}
      ></div>
      <Checkbox
