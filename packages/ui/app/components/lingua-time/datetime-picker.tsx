...
function generateSuggestions(
  ...
  if (!inputValue.length) {
    return defaultSuggestions.map((s) => ({
      date: generateDate(s) as Date,
      inputString: s,
    }))
  }

  const filteredDefaultSuggestions = defaultSuggestions.filter((s) =>
    s.toLowerCase().includes(inputValue.toLowerCase())
  )
  if (filteredDefaultSuggestions.length) {
    return filteredDefaultSuggestions.map((s) => ({
      date: generateDate(s) as Date,
      inputString: s,
    }))
  }

  return [suggestion].filter(isDefinedSuggestion)
}
...
function isDefinedSuggestion(s: Suggestion | null): s is Suggestion {
  return s !== null && s.date !== null
}
  ...
  ({ dateTime, setDateTime, ...props }, ref) => {
    ...
    function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
      if (e.key === "ArrowDown") {
        ...
        if (!selected) return
        const dateStr = generateDateString(selected.date)
        setInputValue(dateStr)
        setDateTime(selected.date)
        closeDropdown()
